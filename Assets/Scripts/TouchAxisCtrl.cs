...
public class TouchAxisCtrl : MonoBehaviour
{
    ...
    void HandleValidTouch(Vector2 currentPos)
    {
        ...
        {
            node.position = transform.position + (Wrj.Utils.ToVector3(currentPos) - transform.position).normalized * GetScaledParimeter(nodeRange);
        }
        else
        {
            node.position = currentPos;
        }
        m_Axis = node.localPosition.normalized * Wrj.Utils.Remap(GetPointDistance(currentPos), 0, GetScaledParimeter(touchArea), 0, 1);
    ...
    public float GetAxis(string vertOrHorizontal)
    {
        if (vertOrHorizontal == "Horizontal")
            ...
            return Wrj.Utils.Remap(m_Axis.x, -1, 1, 1, -1);
        else if (vertOrHorizontal == "InverseVertical")
            return Wrj.Utils.Remap(m_Axis.y, -1, 1, 1, -1);
        else
            return 0f;
    }
